...
class _AccountScreenState extends State<AccountScreen> {
  ...
  Future<void> _removeProfilePicture() async {
    try {
      final user = await RepositoryProvider.instance.updateProfile(name: _name.text.trim(), phone: _phone.text.trim(), avatar: null);
      // remove avatar directly for Supabase-backed repository
      try {
        final repo = RepositoryProvider.instance;
        // No-op for demo; Supabase repository exposes remove through empty avatar fields in SQL update not interface.
      } catch (_) {}
      if (mounted) setState(() => _user = user.copyWith(avatarUrl: ''));
      widget.onProfileChanged?.call();
      showMmSnack(context, 'Profile picture removed locally. Upload a new picture anytime.');
    } catch (e) { showMmSnack(context, friendlyError(e), error: true); }
  }
  ...
  Future<void> _showAvatarOptions() async {
    await showModalBottomSheet<void>(
      ...
      builder: (context) => Container(
        ...
        decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(28), boxShadow: [BoxShadow(color: MmColors.roseDark.withOpacity(.18), blurRadius: 26, offset: const Offset(0, 14))]),
        child: SafeArea(child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.stretch, children: [
          const SectionTitle('Profile picture', subtitle: 'Update how your account appears in Memory Maker.'),
          const SizedBox(height: 14),
          FilledButton.icon(onPressed: () { Navigator.pop(context); _saveProfile(avatar: true); }, icon: const Icon(Icons.upload_rounded), label: const Text('Upload new profile picture')),
          const SizedBox(height: 10),
          OutlinedButton.icon(onPressed: () { Navigator.pop(context); _saveProfile(avatar: true); }, icon: const Icon(Icons.crop_rounded), label: const Text('Edit / replace picture')),
          const SizedBox(height: 10),
          OutlinedButton.icon(onPressed: () { Navigator.pop(context); _removeProfilePicture(); }, icon: const Icon(Icons.delete_outline), label: const Text('Remove picture')),
        ])),
      ),
    );
  }
  ...
  Widget build(BuildContext context) => Scaffold(
    ...
    body: MmGradientBackground(child: _loading ? const Center(child: CircularProgressIndicator()) : RefreshIndicator(onRefresh: _load, child: ListView(padding: const EdgeInsets.all(18), children: [
      MmCard(child: Column(children: [
        Stack(children: [
          InkWell(onTap: _showAvatarOptions, borderRadius: BorderRadius.circular(60), child: CircleAvatar(radius: 44, backgroundColor: MmColors.blush, backgroundImage: imageProviderFromValue(_user?.avatarUrl), child: imageProviderFromValue(_user?.avatarUrl) == null ? const Icon(Icons.person, color: MmColors.roseDark, size: 42) : null)),
          Positioned(right: 0, bottom: 0, child: InkWell(onTap: _showAvatarOptions, child: const CircleAvatar(radius: 17, backgroundColor: MmColors.roseDark, child: Icon(Icons.camera_alt, size: 16, color: Colors.white))))
        ]),
      ...
      MmCard(child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        ...
        if (_pinEnabled) OutlinedButton.icon(onPressed: () async { final pin = await _showPinDialog(); if (pin != null) { await _security.setPin(pin); showMmSnack(context, 'PIN reset successfully.'); } }, icon: const Icon(Icons.restart_alt), label: const Text('Reset PIN')),
        const SizedBox(height: 8),
        SwitchListTile(
          contentPadding: EdgeInsets.zero,
